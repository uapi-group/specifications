...
* Optionally, a firmware image in the `.fwauto` section. This firmware image is an opaque binary blob that can be used as firmware in the next reset. It can also be a container like IGVM that has the firmware contained in it. If multiple `.fwauto` sections exist then one of them is selected according to an implementation-specific algorithm.

...
* `uki-url` is similar to `uki`, but takes an RFC 3986 URI instead of a file path. It may be used to
  ...
  network booting, this entry should be hidden. Similar, if the `:` syntax is used, but the boot loader was
  not invoked via network booting (or doesn't know its originating URL) entries of this type shall be hidden.

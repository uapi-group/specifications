---
title: "UAPI.15 OSC 3008: Hierarchical Context Signalling"
category: Concepts
layout: default
version: 1.0
...
process is systemd's `run0` then primary control is passed to the privileged session of
the target user. If `systemd-nspawn` is then invoked to start a container, primary
...
7. A "breadcrumb" style display can be implementing shown the nesting of contexts to the user.
